...
import torch
import numpy as np
from apps.evabyte.train import TrainArgs
...
class BPBEvalArgs:
    ...
    evaluate_on_raw_only: bool = False
...
def load_model_and_tokenizer_and_train_cfg(
    original_ckpt_path,
    model_cls=EvaByte,
    model_args_cls=EvaByteModelArgs,
    cfg=None
):
    ...
    tokenizer = build_tokenizer(
        ...
        getattr(train_cfg.data.tokenizer, "spm_byte_path", None),
        asdict(train_cfg.data.tokenizer.byte_converter_config) if hasattr(train_cfg.data.tokenizer, "byte_converter_config") else None,
        getattr(train_cfg.data.tokenizer, "separate_embedding", False),
    )
...
def doc_mask_iter(
    ...
    for (batch, loss_mask) in data_loader:
        ...
        if apply_doc_boundary_mask:
            ...
            position_ids = None
        yield (batch, loss_mask, token_types, chunked_token_type_ids, intra_chunk_mask, position_ids)
...
def pack_tokens_with_stride_iter(
    ...
    # all tokens with id < offset are not counted for loss calculation
    if isinstance(tokenizer, BytePairEncodingTokenizer):
        num_effective_tokens = len([token for token in buffer if token < tokenizer.offset])
    else:
        num_effective_tokens = len([token for token in buffer if token >= tokenizer.offset])
    ...
    while True:
        ...
        if end_idx + additional_seq_len > len(buffer):
            ...
            if isinstance(tokenizer, BytePairEncodingTokenizer):
                loss_mask[labels >= tokenizer.offset] = 0
            else:
                loss_mask[labels < tokenizer.offset] = 0
...
def val_data_iter(
    ...
    data_iter = doc_mask_iter(data_iter, chunk_size, window_size, eos_id, apply_doc_boundary_mask)
...
def launch_eval(cfg: BPBEvalArgs):
    with ExitStack() as context_stack:
        ...
        # model, tokenizer, dumped_train_cfg = load_consolidated_model_and_tokenizer(
        #     consolidate_path,
        #     model_cls=EvaByte,
        #     model_args_cls=EvaByteModelArgs,
        # )
        # default_train_cfg = OmegaConf.structured(TrainArgs())
        # train_cfg = OmegaConf.merge(default_train_cfg, dumped_train_cfg)
        # train_cfg = OmegaConf.to_object(train_cfg)
        ...
        if not hasattr(tokenizer, "offset"):
            assert isinstance(tokenizer, BytePairEncodingTokenizer)
            # set the offset to be the first special token ID
            tokenizer.offset = tokenizer.bos_id
        ...
        if (
            hasattr(_data_cfg, "raw_compression_mix_option") and
            _data_cfg.raw_compression_mix_option == "none"
        ):
            _data_cfg.raw_compression_mix_option = "vanilla"

        if _data_cfg.seq_len < cfg.context_seqlen:
            logger.info(f"Training-time sequence length is shorter than validation specification. Casting cfg.context_seqlen to {_data_cfg.seq_len} and cfg.context_stride to {_data_cfg.seq_len}")
            cfg.context_seqlen = _data_cfg.seq_len
            cfg.context_stride = cfg.context_seqlen

        if cfg.evaluate_on_raw_only:
            logger.info("Evaluating on raw data only")
            if _data_cfg.raw_compression_mix_option != "vanilla":
                _data_cfg.raw_compression_mix_option = "sentinel"
            _data_cfg.compression_sampling_rate = 0.0
            _data_cfg.compression_alg_config = "gzip"
            _data_cfg.tokenizer.name = "hf"
            _data_cfg.tokenizer.spm_byte_path = None
        ...
        val_data_loader, num_bytes = val_data_iter(
            ...
            chunk_size=train_cfg.model.chunk_size,
            window_size=train_cfg.model.window_size,
        ...
        while True:
            ...
            with torch.no_grad():
                try:
                    batch, loss_mask, token_types, chunked_token_type_ids, intra_chunk_mask, position_ids = next(val_data_loader)
                ...
                end_timer.record()
                if train_cfg.model.num_pred_heads > 1:
                    # NOTE: in case of multibyte pred,
                    # we only use the first head to calculate the loss
                    logits = logits.view(
                        logits.shape[0], 
                        logits.shape[1], 
                        train_cfg.model.num_pred_heads, 
                        train_cfg.model.vocab_size
                    )[:, :, 0, :]
