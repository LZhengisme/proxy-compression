...
from .triton_op_utils import triton_op
...
_capability_90_config = {
    "fwd": {
        (torch.bfloat16, 64, 0): (128, 128, 4, 3), 
        (torch.bfloat16, 128, 0): (128, 128, 8, 3),
        (torch.float32, 64, 0): (128, 64, 8, 3),
        (torch.float32, 128, 0): (64, 32, 4, 3),
        (torch.bfloat16, 64, 1): (128, 128, 4, 2), 
        (torch.bfloat16, 128, 1): (128, 128, 8, 2),
        (torch.float32, 64, 1): (128, 64, 8, 2),
        (torch.float32, 128, 1): (64, 32, 4, 2),
    },
    "bwd_dq": {
        (torch.bfloat16, 64, 0): (128, 64, 4, 3),
        (torch.bfloat16, 128, 0): (128, 64, 8, 3),
        (torch.float32, 64, 0): (128, 64, 8, 2),
        (torch.float32, 128, 0): (32, 32, 4, 2),
        (torch.bfloat16, 64, 1): (128, 64, 4, 3),
        (torch.bfloat16, 128, 1): (128, 64, 8, 3),
        (torch.float32, 64, 1): (128, 64, 8, 2),
        (torch.float32, 128, 1): (32, 32, 4, 2),
    },
    "bwd_dkdv": {
        (torch.bfloat16, 64, 0): (128, 64, 4, 2),
        (torch.bfloat16, 128, 0): (128, 64, 8, 2),
        (torch.float32, 64, 0): (128, 64, 8, 2),
        (torch.float32, 128, 0): (32, 32, 4, 1),
        (torch.bfloat16, 64, 1): (128, 64, 4, 2),
        (torch.bfloat16, 128, 1): (128, 64, 8, 2),
        (torch.float32, 64, 1): (128, 64, 8, 2),
        (torch.float32, 128, 1): (32, 32, 4, 1),
    },
    "bwd_drfa_kv": {
        (torch.bfloat16, 64, 0): (128, 64, 4, 2),
        (torch.bfloat16, 128, 0): (128, 64, 8, 2),
...
def _get_config(dtype, head_dim, mask_type, mode) -> tuple[int, int, int, int]:
    capability = torch.cuda.get_device_capability()
    if capability >= (9, 0):
        kernel_config = _capability_90_config[mode].get((dtype, head_dim, mask_type), (32, 32, 4, 1))
    elif capability >= (8, 0):
        # NOTE: no mask type dispatch
...
@triton_op(
    "eva::eva_agg_fwd", 
    mutates_args={},
    schema="(Tensor q, Tensor k, Tensor v, Tensor? rfa_k, Tensor? rfa_v, Tensor? window_mask, Tensor? chunk_mask, float? softmax_scale, int window_size, int chunks_per_window) -> (Tensor o, Tensor lse)",
...
def triton_eva_agg_fwd(
    ...
    BLOCK_M, BLOCK_N, num_warps, num_stages = _get_config(q.dtype, head_dim, mask_type, "fwd")
    ...
    wrap_triton(_fwd_eva_agg_kernel)[grid](
        q,
        k,
        v,
        rfa_k,
        ...
        EVEN_C=nchunks % BLOCK_N == 0,
        EVEN_W=window_size % BLOCK_N == 0,
        EVEN_HEADDIM=head_dim == BLOCK_HEADDIM,
        BLOCK_M=BLOCK_M,
        BLOCK_N=BLOCK_N,
        num_warps=num_warps,
...
@triton_op(
    "eva::eva_agg_bwd", 
    mutates_args={},
    schema="(Tensor do, Tensor q, Tensor k, Tensor v, Tensor? rfa_k, Tensor? rfa_v, Tensor? window_mask, Tensor? chunk_mask, Tensor o, Tensor lse, float? softmax_scale, int window_size, int chunks_per_window, int empty_rfa_kv, int mask_type) -> (Tensor dq, Tensor dk, Tensor dv, Tensor d_rfa_k, Tensor d_rfa_v)",
    allow_decomposition=True,
)
def triton_eva_agg_bwd(
    ...
    BLOCK_M, BLOCK_N, num_warps, num_stages = _get_config(q.dtype, head_dim, mask_type, "bwd_dq")
    ...
    wrap_triton(_bwd_eva_agg_kernel_dq)[grid](
        q,
        k,
        v,
        rfa_k,
    ...
    BLOCK_M, BLOCK_N, num_warps, num_stages = _get_config(q.dtype, head_dim, mask_type, "bwd_dkdv")
    grid = lambda META: (
        triton.cdiv(seqlen_k, META["BLOCK_N"]),
        batch * nheads,
    )
    wrap_triton(_bwd_eva_agg_kernel_dkdv)[grid](
        q,
        k,
        v,
        window_mask,
    ...
    if empty_rfa_kv == 0:
        BLOCK_M, BLOCK_N, num_warps, num_stages = _get_config(q.dtype, head_dim, mask_type, "bwd_drfa_kv")
        grid = lambda META: (
            triton.cdiv(nchunks, META["BLOCK_N"]),
            batch * nheads,
        )
        wrap_triton(_bwd_eva_agg_kernel_drfa_kv)[grid](
            q,
            rfa_k,
            rfa_v,
            chunk_mask,
    ...
    return dq, dk, dv, d_rfa_k, d_rfa_v
...
class EvaAggFunc(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        dq, dk, dv, d_rfa_k, d_rfa_v = triton_eva_agg_bwd(
            do,
            q,
            k,
            v,
