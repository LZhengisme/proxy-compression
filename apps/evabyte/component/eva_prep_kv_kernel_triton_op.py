...
@triton_op(
    "eva::eva_prep_kv_fwd", 
    mutates_args={},
    schema="(Tensor k, Tensor v, Tensor param_mu, Tensor param_phi, Tensor? mask, float? softmax_scale, int chunksize) -> (Tensor rfa_k, Tensor rfa_v)",
...
def triton_eva_prep_kv_fwd(k, v, param_mu, param_phi, mask, softmax_scale, chunksize):
    ...
    wrap_triton(_fwd_eva_prep_kv_kernel)[grid](
        k,
        v,
        param_mu,
        param_phi,
...
@triton_op(
    "eva::eva_prep_kv_bwd", 
    mutates_args={},
    schema="(Tensor d_rfa_k, Tensor d_rfa_v, Tensor k, Tensor v, Tensor param_mu, Tensor param_phi, Tensor? mask, Tensor rfa_k, Tensor rfa_v, float? softmax_scale, int mask_type, int chunksize) -> (Tensor d_k, Tensor d_v, Tensor d_param_mu, Tensor d_param_phi)",
    allow_decomposition=True,
)
def triton_eva_prep_kv_bwd(
    ...
    wrap_triton(_bwd_eva_prep_kv_kernel)[grid](
        rfa_k, # [b, h, c, d]
        rfa_v, # [b, h, c, d]
        k, # [b, h, n, d]
        v, # [b, h, n, d]
    ...
    return d_k, d_v, d_param_mu, d_param_phi
...
class EvaPrepKVFunc(torch.autograd.Function):
    ...
    def backward(ctx, d_rfa_k, d_rfa_v):
        ...
        d_k, d_v, d_param_mu, d_param_phi = triton_eva_prep_kv_bwd(
            d_rfa_k, d_rfa_v,
            k, v, param_mu, param_phi, 
            mask, 
            rfa_k, rfa_v,
