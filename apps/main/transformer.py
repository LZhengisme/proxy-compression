...
class LMTransformer(BaseTransformer):
    ...
    def forward(
        ...
        mask = (
            mask
            if (mask is not None) or (cuseq_lens is not None)
            else create_causal_mask(seqlen, attn_impl, self.sliding_window)
        )
        h = super().forward(h, tok_idx=tok_idx, mask=mask, attn_impl=attn_impl, cuseq_lens=cuseq_lens, max_seqlen=max_seqlen)
