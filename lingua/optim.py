...
def lr_linear_decay(
    ...
    if step < start_decay_step:
        lr = 1.0
    elif step >= start_decay_step and step <= end_decay_step:
        decay_length = end_decay_step - start_decay_step
        slope = -(1.0 - min_ratio) / decay_length
        intercept = 1.0 + ((1.0 - min_ratio) * start_decay_step) / decay_length
        lr = slope * step + intercept
    else:
        lr = min_ratio
    return lr
