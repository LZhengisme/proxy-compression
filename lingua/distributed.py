...
class DistributedArgs:
    ...
    compile_cache_size_limit: int = 128
...
def setup_env(env_args):
    ...
    # if get_is_torch_run():
    #     # otherwise it seems to cause errors like invalid device ordinal
    #     env_vars["ENABLE_INTRA_NODE_COMM"] = "0"
...
def _apply_ac_to_layer(module: nn.Module, no_recompute_ops, sac_config: SACConfig):
    valid_ac_modes = ("full", "selective")
    if sac_config.mode not in valid_ac_modes:
        raise ValueError(
            f"Invalid AC mode: {sac_config.mode}. Valid modes: {valid_ac_modes}"
        )

    if sac_config.mode == "full":
        return checkpoint_wrapper(module, preserve_rng_state=False)

    assert sac_config.mode == "selective", f"{sac_config.mode}"
    use_op_sac = sac_config.selective_ac_option == "op"
    use_layer_sac = sac_config.selective_ac_option.isdigit()
    if not use_op_sac and not use_layer_sac:
        raise ValueError(
            f"Invalid selective AC option: {sac_config.selective_ac_option}. "
    ...
    if use_op_sac:
        def _get_custom_policy(meta):
            def _custom_policy(ctx, func, *args, **kwargs):
                ...
                if func == torch.ops.aten.mm.default:
                    meta[mm_count_key] += 1
                _save_list = no_recompute_ops or default_no_recompute_ops
                # Saves output of all compute ops, except every second mm
                to_save = (func in _save_list) and not (
                    func == torch.ops.aten.mm.default and 
                    meta[mm_count_key] % 2 == 0
                )
                return (
        ...
        ac_freq = int(sac_config.selective_ac_option)
...
def apply_ac(model: nn.Module, no_recompute_ops, sac_config: SACConfig):
    """Apply activation checkpointing to the model."""
    for layer_id, layer in enumerate(model.layers):
        layer = _apply_ac_to_layer(layer, no_recompute_ops, sac_config)
        model.layers[layer_id] = layer

    logger.info(f"Applied {sac_config.mode} activation checkpointing to the model")

def apply_compile(model: nn.Module):
    """
    Apply torch.compile to each layer, which makes compilation efficient due to
    ...
    logger.info("Compiling each layer with torch.compile")
...
def parallelize_model(
    ...
    if distributed_args.selective_activation_checkpointing:
        apply_ac(model, no_recompute_ops, distributed_args.sac_config)
    ...
    if (
        ...
        # set the root module's cast_forward_inputs to false so that
        # loss masks do not get cast to bf16
        entry_fsdp_config = dict(
            mp_policy=(
                MixedPrecisionPolicy(
                    param_dtype=param_dtype,
                    reduce_dtype=torch.float32,
        ...
        model = fully_shard(model, **entry_fsdp_config, reshard_after_forward=True)
    else:
