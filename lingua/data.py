...
from torch.multiprocessing import Process, Queue, Event, SimpleQueue
from queue import Full, Empty
import os
from pathlib import Path
from queue import Full
from typing import Dict, Any, Iterator, Optional, TypedDict, List
...
from lingua.neural_compression_utils import (
    pack_compressed_spans,
...
class FormatOption(Enum):
    NO_PAIRING = "none"
    TRANSLATED_RAW_COMPRESSED = "translated_r_c"
    TRANSLATED_COMPRESSED_RAW = "translated_c_r"
    PARALLEL_COMPRESSED_RAW = "parallel_c_r"
    PARALLEL_RAW_COMPRESSED = "parallel_r_c"
...
class JSONLState(TypedDict):
    ...
    Attributes:
        ...
        current_iter (Optional[int]): Number of iterations over the entire jsonl file (for infinite iteration).
    """
    ...
    offset: int
    current_iter: int
    compression_rng_state: Optional[Any]
    compression_sampling_rate: Optional[float]
    raw_compression_mix_option: Optional[str]
...
class MultiChoiceState(TypedDict):
    ...
    Attributes:
        ...
        source_to_state: Dict[str, Any] Dict from source to iterator state
...
class TokenizerState(TypedDict):
    ...
    add_eos: bool
    path: Optional[str]
    spm_byte_path: Optional[str]
    byte_conversion_args: Optional[Dict]
...
def read_jsonl(
    ...
    compression_sampling_rate_state: Optional[Any] = None,
):
    ...
    if compression_rng_state is None:
        # Optionally, you could derive a seed from file_path and offset for reproducibility.
        rng = np.random.default_rng()
    else:
        rng = np.random.default_rng()
        rng.bit_generator.state = compression_rng_state
    ...
    state = JSONLState(
        ...
        compression_rng_state=compression_rng_state,
        compression_sampling_rate=compression_sampling_rate,
        raw_compression_mix_option=raw_compression_mix_option,
    )
        ...
        while line := file.readline():
            ...
            if (current_line - 1) % block_size == offset:
                ...
                state = JSONLState(
                    ...
                    offset=offset,
                    current_iter=current_iter,
                    compression_rng_state=rng.bit_generator.state,
                    compression_sampling_rate=state["compression_sampling_rate"],
                    raw_compression_mix_option=state["raw_compression_mix_option"],
                )
                # DEBUG
                try:
                    data = json.loads(line)
                except json.JSONDecodeError as e:
                    logger.warning(f"file: {file_path} position: {current_line} Could not decode JSON: {line} with error {e}")
                    continue
                content_key = "text" if ("text" in data) else "content"

                if compression_sampling_rate_state is None:
                    compression_sampling_prob = state["compression_sampling_rate"]
                    compression_sampling_mix_option = state["raw_compression_mix_option"]
                else:
                    cur_tup = compression_sampling_rate_state.get_compression_sampling_rate_mode()
                    compression_sampling_prob, compression_sampling_mix_option = cur_tup
                if compression_sampling_mix_option == "vanilla":
                    # raw and compressed data are both present and randomly sampled
                    add_sentinel = False
                    format_option = FormatOption.NO_PAIRING
                    format_choice = FormatChoice.NONE
                elif compression_sampling_mix_option == "sentinel":
                    # both raw and compressed data are present
                    # but they are wrapped with a sentinel special token
                    # to indicate which one is which
                    # raw: <raw>...</raw>
                    # compressed: <compressed>...</compressed>
                    add_sentinel = True
                    format_option = FormatOption.NO_PAIRING
                    format_choice = FormatChoice.NONE
                elif compression_sampling_mix_option == "mix_translation_raw":
                    # 50% prob. output raw
                    # <raw>...</raw>
                    # 50% prob. output random translation
                    # { 
                    ...
                    add_sentinel = True
                    if rng.random() < 0.25:
                        format_choice = FormatChoice.RAW
                        format_option = FormatOption.TRANSLATED_RAW_COMPRESSED
                    elif rng.random() < 0.5:
                        format_choice = FormatChoice.RAW
                        format_option = FormatOption.TRANSLATED_COMPRESSED_RAW
                    else:
                        format_choice = FormatChoice.NONE
                        format_option = FormatOption.NO_PAIRING
                elif compression_sampling_mix_option in [
                    ...
                    # NOTE: we need to calibrate the sampling prob, as whenever there is 
                    # a raw byte stream sampled, it comes with a compressed byte stream.
                    # so to keep the probability of compressed bytes to be x,
                    # we need calibrated rate y such that y + (1-y) : (1-y) = x : 1-x
                    if compression_sampling_prob > 0.5:
                        # we will transform all raw bytes to (raw, compressed) pairs
                        compression_sampling_prob = 2. - (1. / compression_sampling_prob)
                        format_choice = FormatChoice.RAW
                    else:
                        # we will transform all compressed bytes to (compressed, raw) pairs
                        compression_sampling_prob = compression_sampling_prob / (1. - compression_sampling_prob)
                        format_choice = FormatChoice.COMPRESSED
                    add_sentinel = True
                    if compression_sampling_mix_option == "translation_raw_compressed":
                        # both raw and compressed data are present
                        # but raw has its compressed translation **appended**
                        # raw: <raw>...</raw><compressed>...</compressed>
                        # compressed: <compressed>...</compressed>
                        format_option = FormatOption.TRANSLATED_RAW_COMPRESSED
                    elif compression_sampling_mix_option == "translation_compressed_raw":
                        # both raw and compressed data are present
                        # but raw has its compressed translation **prepended**
                        # raw: <compressed>...</compressed><raw>...</raw>
                        # compressed: <compressed>...</compressed>
                        format_option = FormatOption.TRANSLATED_COMPRESSED_RAW
                    elif compression_sampling_mix_option == "translation_random":
                        # raw has its compressed translation 
                        # **prepended** or **appended** with a probability
                        # raw: 
                        #   { 
                        #   <compressed>...</compressed><raw>...</raw>
                        #   <raw>...</raw><compressed>...</compressed>
                    ...
                    elif compression_sampling_mix_option == "parallel_compressed_raw":
                        # both raw and compressed data are present
                        # but raw has its compressed translation **prepended**
                        # note we treat these two version as if they were from different samples
                        # raw: <compressed>...</compressed><raw>...</raw>
                        # compressed: <compressed>...</compressed>
                        format_option = FormatOption.PARALLEL_COMPRESSED_RAW
                    elif compression_sampling_mix_option == "parallel_raw_compressed":
                        # both raw and compressed data are present
                        # but raw has its compressed translation **appended**
                        # note we treat these two version as if they were from different samples
                        # raw: <raw>...</raw><compressed>...</compressed>
                        # compressed: <compressed>...</compressed>
                        format_option = FormatOption.PARALLEL_RAW_COMPRESSED
                    elif compression_sampling_mix_option == "parallel_random":
                        # raw has its compressed translation 
                        # **prepended** or **appended** with a probability
                        # raw: 
                        #   { 
                    ...
                    raise ValueError(f"Unknown raw_compression_mix_option: {state['raw_compression_mix_option']}")
                data_utf8 = data[content_key].encode("utf-8")

                is_compressed = rng.random() < compression_sampling_prob

                payload = {
                    "utf8_bytes": data_utf8,
                ...
                for key in data.keys():
                    # NOTE: temp hack to support m1 AC bytes
                    if key.startswith("m1") or key.endswith("ac_b64"):
                        payload[key] = data[key]
                # NOTE: for ensemble AC datasets, randomly choose one compressed source
                all_compressed_sources = [key for key in data.keys() if key.startswith("m1_ac_")]
                if len(all_compressed_sources) > 1:
                    compressed_source = rng.choice(all_compressed_sources)
                    payload["m1_ac_chosen_from_mixture"] = data[compressed_source]
                yield payload, state
...
def loop_on_jsonl(
    ...
    compression_sampling_rate: float,
    raw_compression_mix_option: str,
    compression_rate_state: Optional[Any] = None,
):
    ...
    try:
        while True:
            it = read_jsonl(
                ...
                offset,
                current_iter,
                compression_rng_state,
                compression_sampling_rate,
                raw_compression_mix_option,
                compression_rate_state,
            )
            for content, jsonl_state in it:
                yield content, jsonl_state
                compression_rng_state = jsonl_state["compression_rng_state"]
                compression_sampling_rate = jsonl_state["compression_sampling_rate"]
                raw_compression_mix_option = jsonl_state["raw_compression_mix_option"]
            current_iter += 1
            position = 0
    finally:
        it.close()
...
def tokenize(
    ...
    compression_alg_config: str = "gzip",
    ...
    if spm_byte_path is not None:
        assert compression_alg_config in ["spm_byte"]
    if compression_alg_config in ["spm_byte"]:
        tokenizer = build_tokenizer(
            name=tokenizer_type, 
            path=tokenizer_path,
            spm_byte_path=spm_byte_path,
            byte_conversion_args=byte_conversion_args,
        ...
        tokenizer = build_tokenizer(
            ...
            path=tokenizer_path,
            spm_byte_path=None,
            byte_conversion_args=None,
            separate_embedding=separate_embedding,
        )

    if compression_alg_config == "halfbyte":
        half_byte_codec = HalfByteCodec()
    else:
        half_byte_codec = None

    if compression_alg_config == "doublebit":
        double_bits_codec = DoubleBitCodec()
    else:
        double_bits_codec = None

    if compression_alg_config == "bit":
        bits_codec = BitCodec()
    else:
        bits_codec = None

    if compression_alg_config == "doublebyte":
        double_byte_codec = DoubleByteCodec()
    else:
        double_byte_codec = None

    if compression_alg_config.startswith("ac_m1_key-"):
        _splits = compression_alg_config.split("-", 2)
        assert len(_splits) == 3, f"alg_config must be ac_m1_key-pack_<int>-<actual_dict_key>, got {compression_alg_config}"
        assert _splits[0] == "ac_m1_key"
        assert _splits[1].startswith("pack_"), f"alg_config must be of format ac_m1_key-pack_<int>-<actual_dict_key>, got {compression_alg_config}"
        pack_bytelen = int(_splits[1].split("_")[-1])
        assert pack_bytelen >= 0, f"pack_bytelen must be greater than 0, got {pack_bytelen}"
        m1_ac_key = _splits[-1]
        if pack_bytelen > 0:
            key_splits = m1_ac_key.split("_")
            bit_threshold = None
            for k in key_splits:
                if k.startswith("ow"):
            ...
            ac_pack_fn = partial(
                pack_compressed_spans, 
                bits_per_compressed=pack_bytelen, 
                compression_bit_threshold=bit_threshold, 
    ...
    def compress_bytes(content: dict) -> bytes:
        if compression_alg_config.startswith("ac_m1_key-"):
            mixed_raw_compressed_bytes = packed_bytes_to_pseudo(base64.b64decode(content[m1_ac_key]))
            if ac_pack_fn is not None:
                mixed_raw_compressed_bytes = ac_pack_fn(mixed_raw_compressed_bytes)
            return mixed_raw_compressed_bytes
        else:
            data_bytes = content["utf8_bytes"]
        if compression_alg_config == "gzip":
            return gzip.compress(data_bytes)
        elif compression_alg_config == "gzip_no_mtime":
            return gzip.compress(data_bytes, mtime=0)
        elif compression_alg_config == "spm_byte":
            data_str = data_bytes.decode("utf-8")
            return tokenizer.encode_with_spm(data_str)
        elif compression_alg_config == "halfbyte":
            return half_byte_codec.encode(data_bytes)
        elif compression_alg_config == "bit":
            return bits_codec.encode(data_bytes)
        elif compression_alg_config == "doublebit":
            return double_bits_codec.encode(data_bytes)
        elif compression_alg_config == "doublebyte":
            return double_byte_codec.encode(data_bytes)
        else:
            raise ValueError(f"Unknown compression algorithm: {compression_alg_config}")

    def format_data(content: dict, format_option: FormatOption) -> bytes:
        raw_bytes = content["utf8_bytes"]
        if (
            format_option == FormatOption.TRANSLATED_RAW_COMPRESSED or 
            format_option == FormatOption.TRANSLATED_COMPRESSED_RAW
        ):
            raw_tokens = tokenizer.encode_bytes_with_sentinels(
                raw_bytes, 
                bytes_type="raw",
                add_bos=False, 
                add_eos=False
            ...
            compressed_tokens = tokenizer.encode_bytes_with_sentinels(
                compressed, 
                bytes_type="compressed", 
                add_bos=False, 
                add_eos=False
            )
            if format_option == FormatOption.TRANSLATED_RAW_COMPRESSED: 
                tokens = (
                    [tokenizer.bos_id] * add_bos +
                    raw_tokens +
                    compressed_tokens +
                    [tokenizer.eos_id] * add_eos
                )
            else:
                tokens = (
                    [tokenizer.bos_id] * add_bos +
            ...
            format_option == FormatOption.PARALLEL_RAW_COMPRESSED or 
            format_option == FormatOption.PARALLEL_COMPRESSED_RAW
        ):
            raw_tokens = tokenizer.encode_bytes_with_sentinels(
                raw_bytes, 
            ...
            if format_option == FormatOption.PARALLEL_RAW_COMPRESSED: 
                tokens = raw_tokens + compressed_tokens
            else:
                tokens = compressed_tokens + raw_tokens
        else:
            raise ValueError(f"Unknown format_option: {format_option}")
        return tokens

    for content, state in iterator:
        # Tokenize the data, add sentinels, format raw/compressed data if needed
        if content["add_sentinel"]:
            assert compression_alg_config != "vanilla"
            format_option = content["format_option"]
            format_choice = content["format_choice"]
            if content["is_compressed"]:
                if format_option != FormatOption.NO_PAIRING and format_choice == FormatChoice.COMPRESSED:
                    tokens = format_data(content, format_option)
                else:
                    tokens = tokenizer.encode_bytes_with_sentinels(
                        compress_bytes(content), 
                        bytes_type="compressed", 
                        add_bos=add_bos, 
                        add_eos=add_eos
                    )
            else:
                if format_option != FormatOption.NO_PAIRING and format_choice == FormatChoice.RAW:
                    tokens = format_data(content, format_option)
                else:
                    tokens = tokenizer.encode_bytes_with_sentinels(
                        content["utf8_bytes"], 
            ...
            if compression_alg_config == "vanilla":
                # performs vanilla tokenization
                ...
                # is_compressed is sampled based on compression_sampling_prob
                if content["is_compressed"]:
                    data_bytes = compress_bytes(content)
                else:
                    data_bytes = content["utf8_bytes"]
                tokens = tokenizer.encode_bytes(data_bytes, add_bos=add_bos, add_eos=add_eos)

        # print(f"[DEBUG]: {[tokens]}", flush=True)
        yield tokens, TokenizerState(
            ...
            spm_byte_path=spm_byte_path,
            byte_conversion_args=byte_conversion_args,
            compression_alg_config=compression_alg_config,
            separate_embedding=separate_embedding,
        )
...
def pack_tokens(
    ...
    for i, (tokens, state) in enumerate(iterator):
        ...
        # if start_token is invalid after resuming, skip this sequence entirely
        if start_token >= len(tokens):
            logger.warning(f"Invalid start_token {start_token} >= sequence length {len(tokens)} at sequence {i}. Skipping sequence.")
            # Skip this entire sequence and reset for next one
            start_token = 0
...
def init_state(
    ...
    compression_sampling_rate: float = 0.0,
    raw_compression_mix_option: str = "vanilla",
    compression_alg_config: str = "gzip",
    separate_embedding: bool = False,
):
...
def feed_buffer(queue: Queue, stop_event, iterator_builder):
    """
...
@contextlib.contextmanager
def async_iterator(buffer_size: int, iterator_builder, prefill: int = 4, startup_timeout: float = 120.0):
    """
    Context manager to setup and manage asynchronous iteration with producer-consumer model.
    """
    ctx = mp.get_context("spawn")
    queue = ctx.Queue(maxsize=buffer_size)
    stop_event = ctx.Event()
    producer = ctx.Process(target=feed_buffer, args=(queue, stop_event, iterator_builder))
    logger.info("Async dataloader started")
    producer.start()

    # prefilled = []
    # deadline = time.time() + startup_timeout
    # while len(prefilled) < prefill:
    #     remaining_time = deadline - time.time()
    #     if remaining_time <= 0:
    #         producer.join()
    #         raise RuntimeError(
    #             f"Data loader failed to enqueue prefilling batches within {startup_timeout}s (exitcode={producer.exitcode})."
    #         )
    #     try:
    #         item = queue.get(timeout=min(0.5, remaining_time))
    #         prefilled.append(item)
    #     except Empty:
    #         if not producer.is_alive():
    #             producer.join()
    #             raise RuntimeError(
    #                 f"Producer quit unexpectedly with exitcode={producer.exitcode}"
    #             )
    # logger.info("Prefill complete: {} batches ready".format(len(prefilled)))

    # def _consumer():
    #     for it in prefilled:
    #         yield it
    #     yield from consume_buffer(producer, queue)
    ...
    try:
        ...
        consumer.close()
        producer.join(timeout=0.5)  # Waits for the producer to finish
...
def init_dataloader_state_from_args(
    ...
    return init_state(
        ...
        spm_byte_path=args.tokenizer.spm_byte_path,
        byte_conversion_args=asdict(args.tokenizer.byte_converter_config),
        add_bos=args.add_bos,
        add_eos=args.add_eos,
        compression_sampling_rate=args.compression_sampling_rate,
        raw_compression_mix_option=args.raw_compression_mix_option,
        compression_alg_config=args.compression_alg_config,
        separate_embedding=args.tokenizer.separate_embedding
    )
