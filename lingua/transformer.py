...
from typing import Optional, Union, Tuple, Callable, ClassVar
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if attn_impl == "flex_attention":
            ...
            output = output.transpose(1, 2).contiguous()  # B H S D -> B S H D
        elif attn_impl == "flash_attn2":
            if mask == "causal":
                output = flash_attn_func(xq, xk, xv, causal=True)
            elif cuseq_lens is not None:
                xq,xk,xv = map(lambda e: e.view(-1, e.shape[-2], e.shape[-1]), (xq, xk, xv)) # shape according to varlen requirements
                output = flash_attn_varlen_func(
                    xq, xk, xv,
                    cu_seqlens_q=cuseq_lens, cu_seqlens_k=cuseq_lens,
                    max_seqlen_q=max_seqlen, max_seqlen_k=max_seqlen, 
                    causal=True
                )
                output = output.view(bsz, seq_len, self.n_heads, self.head_dim)
        else:
...
class BaseTransformer(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            h = layer(h, freq_cis, tok_idx=tok_idx, mask=mask, attn_impl=attn_impl, cuseq_lens=cuseq_lens, max_seqlen=max_seqlen)
        return h
