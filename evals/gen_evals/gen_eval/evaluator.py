...
class Evaluator:
    ...
    def _build_gen_tok_kwargs(self, task, args):
        ...
        if args.tokenizer_mode == "tokenization":
            print("[DEBUG] overriding sentinels to reflect regular tokenizers.")
        ...
        if task.stop_words:
            ...
            if fnmatch.fnmatch(self.tokenizer.name_or_path, "*OpenCoder*"):
                task.stop_words.append("</s>")
        ...
        if args.force_generate_utf8_bytes_only:
            gen_kwargs["logits_processor"] = LogitsProcessorList([UTF8ByteOnlyLogitsProcessor(320)])
    ...
    def generate_text(self, task_name):
        ...
        ######################################################################
        # Prompt healing
        if self.args.prompt_healing == "heal":
            io_processor.heal_func = partial(
                heal_tokens,
                model=_model, 
                tokenizer=self.tokenizer, 
                spm_tokenizer=self.tokenizer.lingua_tokenizer.sp_tokenizer, 
