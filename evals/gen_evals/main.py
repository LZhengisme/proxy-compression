...
from gen_eval.utils import TokenizerWrapper
...
def parse_args():
    ...
    parser.add_argument(
        "--force_generate_utf8_bytes_only",
        action="store_true",
        default=False
    )
    parser.add_argument(
...
def main():
    ...
    if args.load_generations_path:
        ...
        if args.byte_converter_type is not None:
            byte_conversion_args = {
                "byte_converter_type": args.byte_converter_type,
            }
        else:
            byte_conversion_args = None
        if args.prompt_healing is not None:
            assert args.prompt_healing in ["strip", "heal", "pad_to_even"]
        if args.tokenizer_mode == "default":
            tokenizer = tokenizer
        else:
            tokenizer = TokenizerWrapper(tokenizer, tokenizer_path, args.tokenizer_mode, args.spm_path, byte_conversion_args, args.separate_embedding)
